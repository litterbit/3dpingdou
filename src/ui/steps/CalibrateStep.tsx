...
export function CalibrateStep({ raster, grid, analysis, occupancyPreview, confidence, occupiedCount, showScores, onShowScoresChange, onToggleOccupancy, selectedCell, onSelectCell, alignMode, onToggleAlignMode, onAlign, effectiveThreshold, onThresholdChange, maxScore, maxColors, onMaxColorsChange, onUpdateDimensions, onUpdateGeometry, onRedetect, onBackToCrop, onRecognize }: {
  raster: Raster;
  grid: GridState;
  analysis: OccupancyAnalysis;
  occupancyPreview: PixelMatrix;
  confidence: number;
    ...
    <section className="workspace calibrate-layout">
      <div className="panel image-panel">
        ...
        <div className="binary-stage">
          <PixelPreview matrix={occupancyPreview} onCellClick={onToggleOccupancy} labels={showScores ? analysis.scores.map((score) => score.toFixed(2)) : undefined} />
        </div>
      ...
      <div className="panel controls-panel">
        ...
        <div className="action-column">
          ...
          <button type="button" className="button primary" onClick={onRecognize}>确认占位并填充颜色</button>
        </div>
      </div>
    </section>
  );
